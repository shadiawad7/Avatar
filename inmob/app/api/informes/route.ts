...
// GET - Obtener todos los informes o filtrar por cliente_id
export async function GET(request: NextRequest) {
  try {
    ...
    if (session.rol === "gestor") {
      ...
      if (clienteParam && clienteParam !== session.userId) {
        return NextResponse.json({ error: "No autorizado" }, { status: 403 })
      }
      filterClienteId = session.userId
    } else if (session.rol === "arquitecto") {
      const arq = await sql`
        SELECT id FROM arquitectos WHERE usuario_id = ${session.userId} LIMIT 1
      `
      ...
      // Permite filtrar adicionalmente por cliente si se facilita
      if (clienteParam) {
        filterClienteId = clienteParam
      }
    } else {
    ...
    if (filterClienteId !== null) {
      conditions.push(sql`i.cliente_id = ${filterClienteId}`)
    }
    if (filterArquitectoId !== null) {
      conditions.push(sql`i.arquitecto_id = ${filterArquitectoId}`)
    }
  ...
  try {
    ...
    const [informe] = await sql`
      INSERT INTO informes (
        inmueble_id, estado, coste_estimado_reparacion, resumen_ejecutivo_texto, cliente_id
      ) VALUES (
        ${inmueble.id},
        ${data.informes?.estado || "borrador"},
        ${data.informes?.coste_estimado_reparacion || 0},
        ${data.informes?.resumen_ejecutivo_texto || ""},
        ${data.informes?.cliente_id || null}
      )
      RETURNING id
    `
