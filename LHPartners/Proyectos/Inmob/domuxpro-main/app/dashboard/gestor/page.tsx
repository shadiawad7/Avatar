...
export default function GestorDashboard() {
  ...
  const handleCambiarEstadoAprobacion = async (informeId: number, nuevoEstado: InformeApprovalState) => {
    const actual = informes.find((inf) => inf.id === informeId)
    if (!actual || actual.estado_aprobacion === nuevoEstado || updatingApprovalId === informeId) return

    setUpdatingApprovalId(informeId)
    try {
      ...
      if (!response.ok) {
        ...
        brandToast.error(error?.error || "No se pudo actualizar el estado de aprobación.")
        return
      }

      setInformes((prev) =>
        prev.map((inf) =>
          inf.id === informeId ? { ...inf, estado_aprobacion: nuevoEstado } : inf,
        ),
      )
      brandToast.success(`Informe INF-${informeId} marcado como ${INFORME_APPROVAL_LABELS[nuevoEstado].toLowerCase()}.`)
    } catch (error) {
      console.error("[gestor] Error al actualizar estado_aprobacion:", error)
      brandToast.error("No se pudo actualizar el estado de aprobación.")
    } finally {
    ...
    const fetchInformes = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const parsed: Informe[] = (data.informes ?? []).map((inf: any) => ({
            ...inf,
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                  {informes.map((informe) => (
                    <div
                      key={informe.id}
                      className="flex flex-col gap-3 p-4 border rounded-lg md:flex-row md:items-start md:justify-between"
                    >
                      <div className="flex-1 space-y-2">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          ...
                          <div className="flex items-center gap-2 text-xs text-muted-foreground md:text-sm">
                            ...
                            <Select
                              value={informe.estado_aprobacion}
                              disabled={updatingApprovalId === informe.id}
                              onValueChange={(value) =>
                                handleCambiarEstadoAprobacion(informe.id, value as InformeApprovalState)
                              }
                            >
                              ...
                              <SelectContent>
                                {INFORME_APPROVAL_STATES.map((option) => (
                                  <SelectItem key={option} value={option}>
                                    {INFORME_APPROVAL_LABELS[option]}
                                  </SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
